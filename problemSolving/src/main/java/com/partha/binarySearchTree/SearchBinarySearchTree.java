...
public class SearchBinarySearchTree {
	...
	public static void main(String[] args) {
		...
		System.out.println(search(tree.root, 91));
		
	}
	
	public static boolean search(Node node,int valueToSearch){
		if(node == null)
			return false;
		if(node.value == valueToSearch)
			...
			if(valueToSearch < node.value)
				return search(node.leftChild, valueToSearch);
			else
				return search(node.rightChild, valueToSearch);
		}
