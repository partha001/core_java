...
public class MinimumCostToConnectAllComponents {
	...
	private static class Solution1 {
		public int minCostConnectPoints(int[][] points) {
			...
			PriorityQueue<Map.Entry<Integer, Integer>> queue = new PriorityQueue<>((a, b) -> (a.getKey() - b.getKey())); //PairKey=distance PairValue=pointIndex
			...
			while (visitCount < n) {
				Map.Entry<Integer, Integer> topElement = queue.poll();
				...
				for (int nextNode = 0; nextNode < n; ++nextNode) {
					if (!visited[nextNode]) { //this condition also takes care of not visiting itself or other visited nodes
						...
						queue.add(Map.entry(nextWeight, nextNode));
					}
				}
			}
	...
	private static class LeetcodeOfficialSolution2 {
		public int minCostConnectPoints(int[][] points) {
			...
			// Min-heap to store minimum weight edge at top.
			PriorityQueue<Map.Entry<Integer, Integer>> heap = new PriorityQueue<>((a, b) -> (a.getKey() - b.getKey()));;

			// Track nodes which are included in MST.
			boolean[] inMST = new boolean[n];

			heap.add(Map.entry(0, 0));
			int mstCost = 0;
			int edgesUsed = 0;

			while (edgesUsed < n) {
				Map.Entry<Integer, Integer> topElement = heap.poll();
				...
				for (int nextNode = 0; nextNode < n; ++nextNode) {
					...
					if (!inMST[nextNode]) {
						...
						heap.add(Map.entry(nextWeight, nextNode));
					}
				}
			}
